// Planta Motor-Tacogerador com Feedback Linearization, Controlador Proporcional e Observador
...
static float u = 0; // Sinal de controle
...
static double K[3] = {102909.84909198, 12261.99460636, 212.31975688};
...
void app_main(void) {
    ...
    snprintf(r, sizeof(r), "%d", 2000); // Inicializa a referência com 1550 mV
    ...
    while (1) {
        ...
        v = K[0] * (ref_z[0] - z_hat[0]) + K[1] * (ref_z[1] - z_hat[1]) + K[2] * (ref_z[2] - z_hat[2]);
        ...
        if (u < 1) {
            ...
            // u_bits = (uint32_t) (0.45*LEDC_NUM_BITS + 0.55*LEDC_NUM_BITS/100*u);
            u_bits = u/100*LEDC_NUM_BITS;
